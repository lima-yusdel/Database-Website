...
<%
	//try {
		...
		dbConnection = DriverManager.getConnection(url, info); //Get the connection
			...
			if (session.getAttribute("custRepUserID") != null){
				userID = session.getAttribute("custRepUserID").toString();
				ps.setString(1, userID);
				session.setAttribute("custRepUserID", null);
			} else {
			...
			if(session.getAttribute("flightB") != null)
			{
				...
				if(rsB.next())
				{
					...
					System.out.print("CurrentA: " + capB);
			...
			// One way flights
				...
				System.out.print("SeatNumberA: "+ seatNumberA);
				if ((Integer)seatNumberA == 0){
					//waitlist
					System.out.print(seatNumberA);
